...
class RoutingMiddleware(MiddlewareBase):
    ...
    async def process(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        group = req.headers.get("x-hx-model-group") or self._routing["routing"]["default_group"]
        all_models: List[Dict[str, Any]] = self._registry.get("models", [])
        ...
        if not selected:
            # Fallback to static weights / failover order
            fo = self._routing["routing"].get("failover_order", [])
            payload["model"] = fo[0] if fo else None
        else:
