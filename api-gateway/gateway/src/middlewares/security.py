...
class SecurityMiddleware(MiddlewareBase):
    def __init__(self):
        ...
        allow_dev_key = os.getenv("HX_ALLOW_DEV_KEY", "").lower() in ("true", "1", "yes")
            ...
            if dev_config_path:
                ...
                try:
                    ...
                    path_is_safe = any(
                        ...
                        if allowed_dir.exists() or str(config_path).startswith(str(allowed_dir))
                    )
                    ...
                    if config_path.exists() and not config_path.is_file():
                        raise RuntimeError(f"Security error: '{config_path}' is not a regular file")
                    
                    # Check file size to prevent reading large files
                    if config_path.exists() and config_path.stat().st_size > 10240:  # 10KB limit
                        raise RuntimeError(f"Security error: Config file '{config_path}' is too large (max 10KB)")
                    ...
                    if config_path.exists():
                        ...
                        if self.master_key:
                            ...
                            raise RuntimeError(f"No MASTER_KEY found in development config file: {config_path}")
                            
                except (IOError, OSError, ValueError) as e:
                    raise RuntimeError(f"Failed to read development config file '{dev_config_path}': {e}")
                except Exception as e:
                    raise RuntimeError(f"Security error accessing config file '{dev_config_path}': {e}")
            elif allow_dev_key:
