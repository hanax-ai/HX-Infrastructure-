...
def mock_rag_services(monkeypatch):
    ...
    def mock_validate_chunking_params(chunk_chars, overlap):
        if not (100 <= chunk_chars <= 8000):
            raise ValueError("chunk_chars must be between 100 and 8000")
    ...
    monkeypatch.setattr(
        loader_route, "validate_chunking_params", mock_validate_chunking_params
    )

    monkeypatch.setattr(delete_route, "qdrant_delete_by_ids", mock_delete_by_ids)
    monkeypatch.setattr(delete_route, "qdrant_delete_by_filter", mock_delete_by_filter)
...
class TestCompleteRAGWorkflow:
    ...
    def test_full_rag_lifecycle(self, client, mock_rag_services):
        ...
        response = client.post(
            "/v1/rag/upsert_markdown", json=markdown_payload, headers=headers
        )
        ...
        files = {
            "file": ("integration-test.pdf", BytesIO(pdf_content), "application/pdf")
        }
        pdf_data = {
            "namespace": "docs:integration-test",
        ...
        response = client.post(
            "/v1/rag/upsert_pdf", files=files, data=pdf_data, headers=headers
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/v1/rag/delete/by_ids", json=delete_payload, headers=headers
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/v1/rag/delete/by_namespace", json=namespace_payload, headers=headers
        )
        assert response.status_code == 200
        namespace_delete_data = response.json()
        assert namespace_delete_data["status"] == "ok"
        assert (
            namespace_delete_data["deleted"] == -1
        )  # Count unknown for batch operations
    ...
    def test_authentication_workflow(self, client, mock_rag_services):
        ...
        for method, endpoint, payload in unauthenticated_tests:
            if method == "POST":
                if endpoint == "/v1/rag/upsert_pdf":
                    ...
                    response = client.post(
                        endpoint, files=files, data={"namespace": "test"}
                    )
                else:
        ...
        delete_tests = [
            ...
            (
                ...
                {"must": [{"key": "namespace", "match": {"value": "docs:auth"}}]},
            ),
        ]
    ...
    def test_error_handling_workflow(self, client):
        ...
        validation_tests = [
            ...
            (
                "/v1/rag/upsert_markdown",
                {"text": "test", "namespace": "test", "chunk_chars": 50},
            ),  # Invalid chunk_chars (below minimum 100)
        ...
        for endpoint, payload in validation_tests:
            ...
            ), f"Expected 422 for {endpoint} with invalid payload"
    ...
    def test_content_variations_workflow(self, client, mock_rag_services):
        ...
        for _i, variation in enumerate(markdown_variations):
            description = variation.pop("description")
            response = client.post(
                "/v1/rag/upsert_markdown", json=variation, headers=headers
            )
        ...
        for variation in pdf_variations:
            ...
            response = client.post(
                "/v1/rag/upsert_pdf", files=files, data=data, headers=headers
            )
            assert response.status_code == 200, f"Failed for {description}"
            ...
            assert (
                response_data["upserted"] > 0
            ), f"No documents upserted for {description}"
...
class TestOpenAPIValidation:
    ...
    def test_openapi_generation(self, client):
        ...
        for path in expected_paths:
            assert (
                path in documented_paths
            ), f"Path {path} not found in OpenAPI documentation"
...
class TestRAGPerformance:
    ...
    def test_batch_processing(self, client, mock_rag_services):
        ...
        for i in range(5):
            ...
            response = client.post(
                "/v1/rag/upsert_markdown", json=payload, headers=headers
            )
            assert response.status_code == 200
        ...
        for i in range(5):
            ...
            response = client.post(
                "/v1/rag/delete/by_namespace", json=cleanup_payload, headers=headers
            )
            assert response.status_code == 200
